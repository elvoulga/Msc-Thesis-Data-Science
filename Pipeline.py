...
createCitationsReferences = 0 # STEP 0 : Whether to Create the files containing the citations' and references' concept occurrence information or skip this step
datasetCreate = 1  # STEP 1 : Whether to Create the dataset from initial CSV files or skip this step
...
# CSV file with concept occurrence frequency of the citations for each article (used for citations' feature addition)
citationsConcepts_csv = baseFolder + caseStudy + '_citationsConcepts.csv';
# CSV file with concept occurrence frequency of the references for each article (used for references' feature addition)
refConcepts_csv = baseFolder + caseStudy + '_refConcepts.csv';
...
# Feature prefixes (used to discriminate between tokens and other added features)
feature_prefix = 'AddedFeatureCUI:'
citation_prefix = 'CitationAddedFeature:'
ref_prefix = 'RefAddedFeature:'
...
'''     
    ...
    **************************************************************************************
    ***     Extract and Create the Concept Occurence Citations and References Files    ***
    **************************************************************************************

    This script is intended to take the list of PMIDs of the articles contained in the dataset and create the concept occurence files of citations and references.
    The procedure is split into three steps:
        a) Access Semantic Scholar database through API and retrieve the citation and reference information
        b) Keep only the citations/references that already exist in the dataset and match them with their pmids
        c) Find the frequency of each concept of citations/references, according to MetaMap results and match them with the corresponding article, creating the concept occurrence files. 
'''


if createCitationsReferences:
    print(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'),
          "-->> Start Citation & Reference data creation")
    createCitationReferenceFiles()
    print(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'), "-->> End Citation & Reference data creation")
else:
    print(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'),
          "-->> Citation & Reference data creation skipped")
    ...
    ******************************************************************
    ...
    This script is intended to create and split Distantly Supervised datasets of articles with concept level topic annotations
        Initial steps before running this script to create a data set:
            ...
            4) Create a CSV file (..._concepts.csv) with concepts per article. For each article add a record with the pmid and the list of concepts with corresponding frequency separated by ":" (e.g. 15305790	["C0243140:2","C0002395:1"])
    ...
    This Script combines for each article the classes read from the first CSV and the data (e.g. abstract and title) from the second CSV, creating a Dataset.
        ...
        The remaining articles are split into Training and Testing datasets retaining the Label-Set Distribution if a proportion different from 0.0 is provided in the setting.
'''
...
useCitations = False;  # Whether to add citations' concept occurence features or not
useRefs = False;  # Whether to add references' concept occurence features or not
...
if Transform:
    ...
    tranformTrainingDataset(train_dir, train_dir_vect, useCUIS, useCitations, useRefs, binaryFrequency, Undersample, occurrenceFrequency_csv, citationsConcepts_csv, refConcepts_csv,
...
for test_folder_name in test_folder_names:
    ...
    if TransformMA:
        ...
        tranformTestData(trainDatasets, labeledTest_dir, useCUIS, useCitations, useRefs, occurrenceFrequency_csv, citationsConcepts_csv, refConcepts_csv, feature_prefix, citation_prefix, ref_prefix,
                          vectorizedTest_dir, binaryFrequency)
        print(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'),
