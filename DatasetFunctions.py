...
'''     
        **********************************************************************
        ***     Functions for Creating the Citations and References files  ***
        **********************************************************************
'''

def createCitationReferenceFiles():
    '''
    Reads the pmids from the articles of the dataset and calls all the required functions to download and transform the citations/references into conceprt occurrence files. Saves the final output 
    in files with the proper format for other function to read  
    '''
    fields_csv = 'Data/AD/AD_abstract.csv' 
    fields = pd.read_csv(fields_csv, ";")  # Read the abstracts file to obtain the pmids into a pandas.DataFrame with columns : pmid, abstract
    pmids = [str(pmid) for pmid in fields['pmid']] # Obtain a list of the pmids
    cit_list = []  # To save the lists of citations
    ref_list = []  # To save the lists of references
    id_list = []   # To save the list of Semantic Scholar ids 

    for pmid in pmids:
        temp_cit, temp_ref, temp_id = download_citations_references(pmid) # For each pmid download the necessary citations/references information 
        cit_list.append(temp_cit)
        ref_list.append(temp_ref)
        id_list.append(temp_id)

    citations_df = pd.DataFrame(list(zip(pmids, id_list, cit_list)), columns =['pmid', 'id', 'cits/refs']) # Create a dataframe with the appropriate format for citations
    refs_df = pd.DataFrame(list(zip(pmids, id_list, ref_list)), columns =['pmid', 'id', 'cits/refs'])  # Create a dataframe with the appropriate format for references

    citation_pmids = transformCitRefFiles(citations_df)  # Transform the dataframe to find the citations that exist in the dataset and match them with their pmids
    reference_pmids = transformCitRefFiles(refs_df)  # Transform the dataframe to find the references that exist in the dataset and match them with their pmids    
    #print (citation_pmids)
    #print (reference_pmids)

    citationsConcepts = cuisFreq(citation_pmids)  # Calculates the frequency of each concept of citations for each article
    refConcepts =  cuisFreq(reference_pmids)  # Calculates the frequency of each concept of references for each article
    #print (citationsConcepts)
    #print (refConcepts)

    citationsConcepts.to_csv('Data/AD/AD_citationsConcepts.csv', sep=';', index=False)  # Save the dataframe with citations in a .csv file for other function use
    sh.sed('-i', 's/\'/\"/g', 'Data/AD/AD_citationsConcepts.csv')  # Transform into other function "readable" format
    refConcepts.to_csv('Data/AD/AD_refConcepts.csv', sep=';', index=False)  # Save the dataframe with references in a .csv file for other function use
    sh.sed('-i', 's/\'/\"/g', 'Data/AD/AD_refConcepts.csv')  # Transform into other function "readable" format

def cuisFreq(df):
    '''
    Calculates the frequency of each concept of citations/references, according to MetaMap results (from _concepts.csv) and match them with the corresponding article
    :param df: Dataframe containing the pmids with their corresponding lists of pmids of their citations/references 
    :return: Dataframe containing the pmids of the articles with the corresponding concept occurrence found by their citations/references
    '''
    concepts_csv = 'Data/AD/AD_concepts.csv'  
    concepts = pd.read_csv(concepts_csv, sep = ';', index_col=0) # Read the concepts file to obtain the MetaMap results for each article
    column_list = []
    for index, row in df.iterrows():  # For each article
        dict_list = []
        string_list = []
        li = list(row['pmids_cit/ref'])
        for pmid in li:
            tempdict = {}
            if int(pmid) in concepts.index:
                cuiListString = concepts.loc[int(pmid)]['cuis'];  # e.g. "[ "C2964138:1", "C0441833:5", "C0444706:2", ... ]'
                cuiListString = cuiListString[1: -1].replace('"', '').replace(' ','')  # i.e. 'C2964138:1,C0441833:5,C0444706:2,...'
                cuis = cuiListString.split(',') # i.e. ['C2964138:1','C0441833:5','C0444706:2','...'] as a python list
                for cuiFreq in cuis:  
                    parts = cuiFreq.split(':')  # Create a dictionary with cuis as keys and frequency as values
                    cui = parts[0] 
                    freq = parts[1]
                    tempdict[cui] = int(freq)
            dict_list.append(tempdict)
        result = dict(functools.reduce(operator.add, map(collections.Counter, dict_list)))  # Sum up all frequencies that have the same key, thus the same cui
        for key, value in result.items():   # Create the correct representation for the concept frequency field
            temp = str(key)+":"+ str(value)
            #temp_double = f'"{temp}"'
            string_list.append(temp)
        column_list.append(string_list)
    df['cuis'] = column_list
    df = df[['pmid', 'cuis']]  # Keep the pmids and concept frequency fields for the dataframe
    return df   # Return the new dataframe
    
    
def transformCitRefFiles(df):
    '''
    Keeps only the citations/references that already exist in the dataset and matchesy6t them with their pmids
    :param df: Dataframe containing the pmids, the Semantic Scholar ids of the articles and the citations/references with their Semantic Scholar id
    :return: Same dataframe, but with two new columns, one for the ctiations/references that match the dataset and one a list of their pmids    
    '''
    ids = list(df.id)     # Obtain list of semantic scholar ids
    pmids = list(df.pmid)  # Obtain list of pmids
    new_cit = []
    pmids_connect = []
    for index, row in df.iterrows():  # For every article of the dataset
        AD_citations = []
        pmid_cit = []
        li = list(row['cits/refs'])  
        for cit in li:       # For every citation/reference
            cit = str(cit.strip('u\''))
            #print (cit)
            if cit in ids:   # Check if the citation/reference belong to semantic scholar, corresponding to pmids, ids and if this is positive, keep it
                AD_citations.append(cit)
                index = ids.index(cit)
                pmid_cit.append(pmids[index])
            else:
                continue
        if len(AD_citations)==0:  # If there are no citations/references that match the dataset put 0 in the list
            AD_citations.append(0)
        if len(pmid_cit) == 0:  # Same for pmids
            pmid_cit.append(0)
        new_cit.append(AD_citations)
        pmids_connect.append(pmid_cit)
    df['AD_citations'] = new_cit   # Create new columns with the citations/references and their pmids
    df['pmids_cit/ref'] = pmids_connect
    return df    # Return the new dataframe

def download_citations_references(pmid):
    '''
    Access Semantic Scholar Database, through the API and retrieve all the ids of the citations and references of each article
    :param pmid: pmid of the article from the dataset to search in Semantic Scholar
    :return: Temporary lists that hold the required fields
    '''
    temp_cit = []
    temp_ref = []
    temp_id = []

    url = "https://api.semanticscholar.org/v1/paper/PMID:%s?include_unknown_references=true" % (str(pmid))  # API access to Semantic Scholar, through pmid
    html = urllib.request.urlopen(url)
    data = json.loads(html.read())  # Read JSON object of specific article
  
    if "error" in data:
        return temp_cit, temp_ref, temp_id   # If no JSON object is found, return empty lists
    else:
        temp_id = data["paperId"]
        if len(data["citations"])==0:  # If there are no citations, display an appropriate message
            print ("No citations found!")
        else:
            for cit in data["citations"]:
                temp_cit.append(cit["paperId"])  # Keep the semantic scholar id of ech citation
        if len(data["references"])==0:   # If there are no references display an appropriate message
            print ("No references found!")
        else:
            for ref in data["references"]:
                temp_ref.append(ref["paperId"])  # Keep tha semantic scholar id of each reference

    return temp_cit, temp_ref, temp_id  # Return the required fields for each article
...
def tranformTrainingDataset(input_dir, outut_dir, useCUIS, useCitations, useRefs, binaryFrequency, Undersample, occurrence_csv, citationsConcepts_csv, refConcepts_csv,
                            feature_prefix, citation_prefix, ref_prefix):
    '''
        STEP 2
    ...
    if useCitations:
        # Get a dictionary from CUI to corresponding list of pmidIndexes for articles of the dataset where it has been recognized and corresponding frequencies (separated by a ":")
        conceptToArticleCitations = getConceptToArticleOccurrenceMap(citationsConcepts_csv, training_dataset)

        # for each CUI
        for cui in conceptToArticleCitations.keys():
            # Create a column of zeros for this CUI, with the length of the sample articles
            tmpColumn = np.array([[0]] * len(training_dataset.filenames))
            # For each article where this CUI occurs
            for pmidIndexFreq in conceptToArticleCitations[cui]:
                parts = pmidIndexFreq.split(":")  # e.g. '234:2'
                pmidIndex = int(parts[0])  # i.e. '234'
                freq = int(parts[1])  # i.e. '2'
                # Update the count of occurrences for this article
            ...
            train_counts = hstack((train_counts, tmpColumn))
            # Add CUI in feature names list
            feature_names.append(citation_prefix + cui)

    if useRefs:
        # Get a dictionary from CUI to corresponding list of pmidIndexes for articles of the dataset where it has been recognized and corresponding frequencies (separated by a ":")
        conceptToArticleRefs = getConceptToArticleOccurrenceMap(refConcepts_csv, training_dataset)

        # for each CUI
        for cui in conceptToArticleRefs.keys():
            # Create a column of zeros for this CUI, with the length of the sample articles
            tmpColumn = np.array([[0]] * len(training_dataset.filenames))
            # For each article where this CUI occurs
            for pmidIndexFreq in conceptToArticleRefs[cui]:
                parts = pmidIndexFreq.split(":")  # e.g. '234:2'
                pmidIndex = int(parts[0])  # i.e. '234'
                freq = int(parts[1])  # i.e. '2'
                # Update the count of occurrences for this article
            ...
            train_counts = hstack((train_counts, tmpColumn))
            # Add CUI in feature names list
            feature_names.append(ref_prefix + cui)
...
def tranformTestData(train_dirs, input_dir, useCUIS, useCitations, useRefs, occurrence_csv, citationsConcepts_csv, refConcepts_csv, feature_prefix, citation_prefix, ref_prefix, outut_dir, binaryFrequency):
    '''
        STEP 5
    ...
    for train_name in train_dirs:
        ...
        if useCitations:
            conceptToArticleCitations = getConceptToArticleOccurrenceMap(citationsConcepts_csv, test_dataset)
        if useRefs:
            conceptToArticleRefs = getConceptToArticleOccurrenceMap(refConcepts_csv, test_dataset)
        ...
        for feature in feature_names:
            ...
            if str(feature).startswith(citation_prefix):
                # Update counts for articles where the cui occurs
                featureIndex = feature_names.index(feature)
                cui = str(feature.replace(feature_prefix, ''))
                # If the CUI occurs in some articles
                if cui in conceptToArticleCitations.keys():
                    # For each article where this CUI occurs
                    for pmidIndexFreq in conceptToArticleCitations[cui]:
                        parts = pmidIndexFreq.split(":")  # e.g. '234:2'
                        pmidIndex = int(parts[0])  # i.e. '234'
                        ...
                        if binaryFrequency:
                            ...
                            count[pmidIndex, featureIndex] = freq
            if str(feature).startswith(ref_prefix):
                # Update counts for articles where the cui occurs
                featureIndex = feature_names.index(feature)
                cui = str(feature.replace(feature_prefix, ''))
                # If the CUI occurs in some articles
                if cui in conceptToArticleRefs.keys():
                    # For each article where this CUI occurs
                    for pmidIndexFreq in conceptToArticleRefs[cui]:
                        parts = pmidIndexFreq.split(":")  # e.g. '234:2'
                        pmidIndex = int(parts[0])  # i.e. '234'
